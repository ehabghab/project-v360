...
def main():
		...
		for chunkLength in Lengths:
			...
			for uID in framesWatchedInTile:
				...
				for chunkID in framesWatchedInTile[uID]:
					...
					wastedTileFrames = 0 # total number of wasted frames in a chunk
					...
					#bytes of dropped frames in a tile (unwatched frames)
					for tile in framesWatchedInTile[uID][chunkID]:
						for fId in range(1,framesInTile[uID][chunkID]+1):
							if fId not in framesWatchedInTile[uID][chunkID][tile]:
								if tileFrameSizes[tile].get(baseFrameNum+fId,None) != None:
									wastedTileFrames += tileFrameSizes[tile][baseFrameNum+fId]


					for tile in extraAreaInTileFrame[uID][chunkID]:
						for fovNum in extraAreaInTileFrame[uID][chunkID][tile]:
							if tileFrameSizes[tile].get(baseFrameNum+fovNum,None) != None:
								tileframeSize = tileFrameSizes[tile][baseFrameNum+fovNum]
								extraArea = extraAreaInTileFrame[uID][chunkID][tile][fovNum]
								wastedAreInTileFrames += tileframeSize * (extraArea/(tileWidth*tileHeight))
					waste[uID][chunkID] = wastedTileFrames  + wastedAreInTileFrames

					grayAreaSize = 0
					greenAreaSize = 0
					#size of transmitted (gray) and needed(green).
					for tile in framesWatchedInTile[uID][chunkID]: #each tile in chunk duration
						for fId in range(1,framesInTile[uID][chunkID]+1): #for each frame
							# if frame in tile among the watched frames
							if fId in framesWatchedInTile[uID][chunkID][tile]:
								if tileFrameSizes[tile].get(baseFrameNum+fId,None) != None:
									frameSize = tileFrameSizes[tile][baseFrameNum+fId]
									grayAreaSize += frameSize
									greenAreaSize += frameSize

									#if frame not watched fully. deduct the not watched  part from green
									if tile in extraAreaInTileFrame[uID][chunkID] and fId in extraAreaInTileFrame[uID][chunkID][tile]:
										extraArea = extraAreaInTileFrame[uID][chunkID][tile][fId]
										greenAreaSize -= frameSize * (extraArea/(tileWidth*tileHeight))


					gray[uID][chunkID] =  grayAreaSize + wastedTileFrames
					green[uID][chunkID] = greenAreaSize
			count = 1000.0/chunkLength
			wasteOver1Sec = {}
			for uID in waste:
				wasteOver1Sec[uID] = {}
				for chunkID in waste[uID]:
					chunkSumID = int(math.floor(chunkID/count))
					if wasteOver1Sec[uID].get(chunkSumID,None) == None:
						wasteOver1Sec[uID][chunkSumID] = 0
					wasteOver1Sec[uID][chunkSumID] += waste[uID][chunkID]
			...
			for uID in gray:
				grayOver1Sec[uID] = {}
				greenOver1Sec[uID] = {}
				for chunkID in gray[uID]:
					chunkSumID = int(math.floor(chunkID/count))
					if grayOver1Sec[uID].get(chunkSumID,None) == None:
						grayOver1Sec[uID][chunkSumID] = 0
					if greenOver1Sec[uID].get(chunkSumID,None) == None:
						greenOver1Sec[uID][chunkSumID] = 0

					grayOver1Sec[uID][chunkSumID] += gray[uID][chunkID]
					greenOver1Sec[uID][chunkSumID] += green[uID][chunkID]



			tileResKey = str(tileWidth) +"x"+ str(tileHeight)
			if unBytesAll.get(tileResKey,None) == None:
				unBytesAll[tileResKey] = {}
				chunkIdAll[tileResKey] = {}
				grayAll[tileResKey] = {}
				greenAll[tileResKey] = {}
				chunkIdAllgg[tileResKey]  = {}

			unBytesAll[tileResKey][chunkLength] = list()
			chunkIdAll[tileResKey][chunkLength] = list()
			for uID in wasteOver1Sec:
				for chunkID in wasteOver1Sec[uID]:
					chunkIdAll[tileResKey][chunkLength].append(chunkID)
					waste = float(wasteOver1Sec[uID][chunkID])
					unBytesAll[tileResKey][chunkLength].append(waste/1e6)

			chunkIdAllgg[tileResKey][chunkLength] = list()
			greenAll[tileResKey][chunkLength] = list()
			grayAll[tileResKey][chunkLength] = list()
			for uID in grayOver1Sec:
				for chunkID in grayOver1Sec[uID]:
					chunkIdAllgg[tileResKey][chunkLength].append(chunkID)
					greenAll[tileResKey][chunkLength].append((8.*greenOver1Sec[uID][chunkID])/1e6)
					grayAll[tileResKey][chunkLength].append((8.*grayOver1Sec[uID][chunkID])/1e6)
    ...
    for chunkLength in Lengths:
	    ...
	    pos = l
	    xticks.append(l+lw)
	    for tileRes in sorted(grayAll,reverse=True):
			boxprops = dict(linewidth=2, color=colors[c],facecolor='white')
			boxplot=plt.boxplot(grayAll[tileRes][chunkLength],showfliers=False,positions=[pos],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)
			modifybox(boxplot,grayAll[tileRes][chunkLength],[10,90],2,colors[c],'k',styles[c])
			pos +=lw
			c-=1
	    l+=1
    ...
    xlabels = list()
    for l in Lengths:
		xlabels.append(str(l)+"ms")



    matches=list()
    c = 2
    for tileRes in sorted(unBytesAll,reverse=True):
		tileWidth = int(tileRes.split('x')[0])
		tileHeight = int(tileRes.split('x')[1])
		lab = str(360/tileWidth)+"x"+str(180/tileHeight)
		matches.append(mpatches.Patch(facecolor='white',edgecolor=colors[c], label=lab,hatch=styles[c],linewidth=2))
		c-=1
    plt.yticks([0,.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8],fontweight="bold",size=14)
    plt.ylabel('Data transmitted (mbps)',fontweight="bold",size=14)
    plt.legend(handles= matches, loc='best',prop={'size': 14,'weight':'bold'})
    plt.xlabel('Chunk Length',fontweight="bold",size=14)
    plt.xticks(xticks,xlabels,fontweight="bold",size=14)
    plt.xlim(0)

    plt.title("vID:"+str(video))
    plt.ylim(0,8)
    plt.savefig("graphs/vID "+str(video)+"_Bytes_gray_boxplot.png",dpi=300)
    #plt.show()
    ...
    for chunkLength in Lengths:
	    ...
	    for tileRes in sorted(grayAll,reverse=True):
			boxprops = dict(linewidth=2, color=colors[c],facecolor='white')
			boxplot=plt.boxplot(greenAll[tileRes][chunkLength],showfliers=False,positions=[pos],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)
			modifybox(boxplot,greenAll[tileRes][chunkLength],[10,90],2,colors[c],'k',styles[c])
			pos +=lw
			c-=1
	    l+=1
    ...
    for tileRes in sorted(unBytesAll,reverse=True):
		...
		c-=1
    plt.yticks([0,.5,1,1.5,2,2.5,3,3.5,4,4.5,5,5.5,6,6.5,7,7.5,8],fontweight="bold",size=14)
    plt.ylabel('Data used (mbps)',fontweight="bold",size=14)
    plt.legend(handles= matches, loc='best',prop={'size': 14,'weight':'bold'})
    plt.xlabel('Chunk Length',fontweight="bold",size=14)
    plt.xticks(xticks,xlabels,fontweight="bold",size=14)
    plt.xlim(0)
    plt.title("vID:"+str(video))
    plt.ylim(0,8)
    plt.savefig("graphs/vID "+str(video)+"_Bytes_green_boxplot.png",dpi=300)
    #plt.show()
