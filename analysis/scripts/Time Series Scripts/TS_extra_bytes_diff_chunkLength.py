...
def main():
			...
			for tile in framesWatchedInTile[uID][chunkID]:
				for fId in range(1,framesInTile[uID][chunkID]+1):
					if fId not in framesWatchedInTile[uID][chunkID][tile]:
						if tileSize.get(baseFrameNum+fId,None) != None:
							wastedTileFrames += tileSize[baseFrameNum+fId]
	...
	for uID in wasteOver1Sec:
		...
		plt.subplot(111)
		#plt.scatter(xaxis,ywastage,linewidth=4,marker='v',color='r',label = str(tileWidth)+"X"+str(tileHeight))
	#################################################################################################################
	...
	for uID in framesWatchedInTile:
		...
		for chunkID in framesWatchedInTile[uID]:
			...
			for tile in framesWatchedInTile[uID][chunkID]:
				for fId in range(1,framesInTile[uID][chunkID]+1):
					if fId not in framesWatchedInTile[uID][chunkID][tile]:
						if tileSize.get(baseFrameNum+fId,None) != None:
							wastedTileFrames += tileSize[baseFrameNum+fId]
							if frameTypes[baseFrameNum+fId] == 'I':
								c += 1
			...
			for fovNum in tilesInFOV[uID][chunkID]:
				if tileSize.get(baseFrameNum+fovNum,None) != None:
					if frameTypes[baseFrameNum+fovNum] == 'I':
						c += 1
	...
	for uID in wasteOver1Sec:
		...
		plt.ylim(0)
		#plt.show()
		plt.savefig("graphs/vID "+str(video)+" uID "+str(uID)+"W:"+str(tileWidth)+",H:"+str(tileHeight)+"_multiLenBytes.png",dpi=300)
		#plt.close()
