...
def main():
	...
	video_switch_latency_no_overlap = {}
	video_switch_latency_overlap = {}
	video_ids = []
	for i in range(1,31):
		print("Video:"+str(i))
		video = i
		...
		all_switches = sum_over_users(switch_bytes)
		all_switchest = sum_over_users(switch_bytest)
	
		video_switch_latency_overlap[video] = all_switchest
		video_switch_latency_no_overlap[video] = all_switches
	...
	ticks = []
	for video_id in sorted(video_switch_latency_no_overlap):
		pos = video_id
		if video_id > 14:
			pos = video_id - 2
		ticks.append(pos)
		boxprops = dict(linewidth=2, color='r',facecolor='white')
		boxplot=plt.boxplot(video_switch_latency_no_overlap[video_id],showfliers=False,positions=[pos-.15],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)
		modifybox(boxplot,video_switch_latency_no_overlap[video_id],[10,90],2,'r','k','x')

		boxprops = dict(linewidth=2, color='dodgerblue',facecolor='white')
		boxplot=plt.boxplot(video_switch_latency_overlap[video_id],showfliers=False,positions=[pos+.05],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)	
		modifybox(boxplot,video_switch_latency_overlap[video_id],[10,90],2,'dodgerblue','k','')



	handles = []
	handles.append(mpatches.Patch(edgecolor='r',facecolor='white', label="No overlap",hatch = 'x'))
	handles.append(mpatches.Patch(edgecolor='dodgerblue',facecolor='white', label="Overlap"))

	plt.legend(handles=handles,loc='best',prop={'weight':'bold'})
	plt.xlabel('Video id',size=16)
	plt.ylabel('Average switch latency (KB)',size=16)
	plt.xticks(ticks,ticks,size=16)
	plt.yticks(size=16)
	plt.xlim(.5,28.5)
	plt.ylim(0)
	#plt.grid(True)
	plt.savefig("graphs/switch_latency_total_bytes_boxplot.png",dpi=300,bbox_inches='tight', pad_inches=0.03)
	plt.close()	
