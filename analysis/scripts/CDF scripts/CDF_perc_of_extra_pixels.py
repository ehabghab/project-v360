...
def main():
	...
	video = 1
    chunkLength = 500 #chunk length in millisecond
    ...
    #check that both ways to calcuate extra areas matches.
	'''for uID in extraAreaInTileFrame:
		for chunkID in extraAreaInTileFrame[uID]:
			waste0 = 0
			for tile in extraAreaInTileFrame[uID][chunkID]:
				for fovNum in extraAreaInTileFrame[uID][chunkID][tile]:
					waste0 +=  (tileHeight*tileWidth) - extraAreaInTileFrame[uID][chunkID][tile][fovNum]

			waste1 = 0
			fovSize = 10000
			for fovNum in tilesInFOV[uID][chunkID]:
				waste1 += (tilesInFOV[uID][chunkID][fovNum]*tileWidth*tileHeight) - 10000.0

			if abs(int(waste0) - int(waste1)) > 1.0:
				#print "not matching"
				print waste0 - waste1
				print "======="
	...
	for uID in waste:
		...
		for chunkID in waste[uID]:
			...
			#waste % = waste / ideal(i.e. number of frames * size of FOV)
			wastePerc = (waste[uID][chunkID] / (framesInTile[uID][chunkID]*10000.)) * 100
			wasteOver1Sec[uID][chunkSumID] += wastePerc
	...
	for uID in wasteOver1Sec:
		...
		for chunkID in wasteOver1Sec[uID]:
			...
			ywastage.append(float(wasteOver1Sec[uID][chunkID]))

		sorted_data = np.sort(ywastage)
		...
		plt.subplot(111)
		plt.plot(sorted_data,yvals,linewidth=3,color='b')
		...
		plt.xlabel('% of unncessary pixels',fontweight="bold",size=14)
		plt.ylabel('Fraction of chunks',fontweight="bold",size=14)
		#plt.xlim(-2,chunkID + 2)
		#plt.ylim(0,100)#,1)
		plt.show()
		#plt.savefig("graphs/vID "+str(video)+" uID "+str(uID)+" [width "+str(tileWidth)+" X height"+str(tileHeight)+"], chunk length "+str(chunkLength)+".png",dpi=300)
		#plt.close()
