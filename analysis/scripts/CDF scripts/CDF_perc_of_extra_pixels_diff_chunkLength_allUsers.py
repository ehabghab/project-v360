...
def main():
	...
	plt.plot(sorted_data,yvals,linewidth=3,color='b',label=str(chunkLength)+"ms")
	#######################################################################################################
	...
	sorted_data = np.sort(ywastage)
	yvals=np.arange(len(sorted_data))/float(len(sorted_data)-1)
	plt.plot(sorted_data,yvals,linewidth=3,color='r',label=str(chunkLength)+"ms")
	...
	for uID in waste:
		...
		for chunkID in waste[uID]:
			...
			wasteOver1Sec[uID][chunkSumID] += waste[uID][chunkID]
			noFrameOver1Sec[uID][chunkSumID] += framesInTile[uID][chunkID]
	#print framesInTile[uID]
	#print noFrameOver1Sec[uID]
	for uID in wasteOver1Sec:
		ywastage = list()
		xaxis = list()
		for chunkID in wasteOver1Sec[uID]:
	...
	sorted_data = np.sort(ywastage)
	yvals=np.arange(len(sorted_data))/float(len(sorted_data)-1)
	plt.plot(sorted_data,yvals,linewidth=3,color='g',label=str(chunkLength)+"ms")

	plt.title("vID:"+str(video)+", W:"+str(tileWidth)+",H:"+str(tileHeight))
	...
	#plt.show()
	plt.savefig("graphs/vID:"+str(video)+", W:"+str(tileWidth)+",H:"+str(tileHeight)+"_ms_CDF_all_multiLen.png",dpi=300)
