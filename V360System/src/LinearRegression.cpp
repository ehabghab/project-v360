...
std::vector<std::pair<float, float>> LinearRegression::predict(
  ...
  lrPredictions.push_back(std::make_pair(input[length - 1].first,input[length - 1].second));
  /*Intialization Phase*/
...
void LinearRegression::init(std::vector<std::pair<float, float>>& input) {
    ...
    auto yawMean  = std::accumulate(yawInputDep.begin(), yawInputDep.end(), 0.0) / n;
    auto pitchMean  = std::accumulate(pitchInputDep.begin(), pitchInputDep.end(), 0.0) / n;
    auto indepMean  = std::accumulate(indep.begin(), indep.end(), 0.0) /  n;

    auto indepVecMult = std::inner_product(indep.begin(), indep.end(), indep.begin(), 0.0);
    auto indepYawVecMult = std::inner_product(indep.begin(), indep.end(), yawInputDep.begin(), 0.0);
    auto indepPitchVecMult = std::inner_product(indep.begin(), indep.end(), pitchInputDep.begin(), 0.0);
