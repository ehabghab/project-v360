...
std::vector<std::pair<float, float>> LinearRegression::Model(
  ...
  for (int i = 0; i < (4 * hw_); i++) {
    ...
    if (yawB1 < lower_bound) yawB1 = 0;
    if (yawB0 < lower_bound) yawB0 = 0;
  ...
  for (int i = 0; i < (4 * hw_); i++) {
    int idx = i % 10;  // for accessing index after every epoch
    float pred =
        pitchB0 + pitchB1 * timeSampleInput[idx];  // making the prediction
    pitchErr = pred - pitchInput[idx];             // calculating the error
    pitchB0 = pitchB0 - pitchAlpha * pitchErr;     // updating b0
    pitchB1 =
        pitchB1 - pitchAlpha * pitchErr * timeSampleInput[idx];  // updating b1
    if (pitchB1 < lower_bound) pitchB1 = 0;
    if (pitchB0 < lower_bound) pitchB0 = 0;
