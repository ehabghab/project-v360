...
void Decoder::initCustomFormatContext() {
	...
	avioContext = avio_alloc_context(buffer, bufferSize,
			0, &bufferReader, &read_packet, NULL, NULL);
...
void Decoder::decode(uint8_t* encodedFrame, uint32_t size, std::vector<uint8_t *>& decodedTileFrames)
{
	...
	//		if (ret < 0) {
	//			System.out.printf("Finding stream failed!\n");
	//			throw new IllegalStateException();
	//		}
	//
	...
	int numBytes = av_image_get_buffer_size(AV_PIX_FMT_ARGB,
	...
	while (av_read_frame(formatContext, avPacket) >= 0) {
		...
		av_image_fill_arrays(pFrameARGB->data, pFrameARGB->linesize,
				buffer2, AV_PIX_FMT_ARGB, avCodecContext->width,
		...
		if(ret1 < 0)
		{
			std::cout<<"Could not decode frame"<< frameId <<" - send"<<std::endl;
		...
		if(ret2 < 0)
		{
			std::cout<<"Could not decode frame"<< frameId <<" - receive"<<std::endl;
			continue;
		}
		//			System.out.printf("Frame %d decoded successfully!\n",frameId);
...
//
//	FILE *p_file = NULL;
//	p_file = fopen("/Users/eghabash/Desktop/360 Video/Project-V360/split/YuvW12H12/gop30/encoded_payloadExtract/7_c_6/1.h264","rb");
...
//
//	uint8_t * buffer = (uint8_t *) malloc(size * sizeof(uint8_t)); // Enough memory for the file
//
