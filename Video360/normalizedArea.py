		...
		for chunkLength in Lengths:
			...
			for uID in framesWatchedInTile:
				...
				for chunkID in framesWatchedInTile[uID]:
					...
					#print("num of tiles:"+str(len(framesWatchedInTile[uID][chunkID])))
					#print("====")
					#bytes of dropped frames in a tile (unwatched frames)
					gray[uID][chunkID] += len(framesWatchedInTile[uID][chunkID])
			...
			if grayAll.get(tileResKey,None) == None:
				grayAll[tileResKey] = {}
			...
			for uID in grayOver1Sec:
				for chunkID in grayOver1Sec[uID]:
					grayAll[tileResKey][chunkLength].append((grayOver1Sec[uID][chunkID]*tileWidth*tileHeight)/(360*180.))
					chunkIdAll[tileResKey][chunkLength].append(chunkID)

	#####################################PLOTTING########################################################

    print "WARNING THIS WILL WORK IF THE CHUNK LENGTH EQUAL TO 1000 MSEC ONLY"
    colors = ['g','r','b']
    markers = ['o','x','v']
    for chunkLength in Lengths:
	    ...
	    for tileRes in sorted(grayAll,reverse=True):
			...
			plt.scatter(chunkIdAll[tileRes][chunkLength][:-1],grayAll[tileRes][chunkLength][:-1],linewidth=2,color=colors[c],marker = markers[c],label=label)
			c-=1
	    ...
	    plt.ylabel('Normalized Area',fontweight="bold",size=14)

	    plt.legend(loc='best',ncol=3,prop={'size': 14,'weight':'bold'})
	    plt.xlim(-1,60)
	    plt.ylim(0,1)
	    plt.title("vID:"+str(video)+" userID:"+str(userID)+" (simulation)")
	    plt.savefig("graphs/vID "+str(video)+"_userID "+str(userID)+"_normalizedAreaSim_ts.png",dpi=300)
	    #plt.show()
