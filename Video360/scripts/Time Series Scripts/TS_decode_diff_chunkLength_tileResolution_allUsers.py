...
def main():
    ...
    chunkIdAll = {}
    decodeDelayAll = {} # tileResultion->bandwidth->uID, network delay for switched tiles throught the sessions (chunkID->networkDelay[chunkID])
    lengths = [1000,200] #chunk length in millisecond
    widths = [60,30,15]
    heights = [30,15,10]
    tileResKeys = list()
    ...
    for tileWidth,tileHeight in zip((widths),(heights)):
		for chunkLength in lengths:
		    print (str(tileWidth)+","+str(tileHeight)+":"+str(chunkLength))
				...
				for chunkID in switchedTiles[uID]:
					if len(switchedTiles[uID][chunkID]) != 0:
						...
						for frameId in switchedTiles[uID][chunkID]:
							decodeDelay[uID][chunkID][frameId-1] = len(switchedTiles[uID][chunkID][frameId])


			count = 1000.0/chunkLength
			decodeDelaySec = {}

			for uID in decodeDelay:
				decodeDelaySec[uID] = {}
				for chunkID in decodeDelay[uID]:
			...
			if chunkIdAll.get(tileResKey,None) == None:
				chunkIdAll[tileResKey] = {}
				decodeDelayAll[tileResKey] = {}

			chunkIdAll[tileResKey][chunkLength] = list()
			decodeDelayAll[tileResKey][chunkLength] = list()

			for uID in decodeDelaySec:
				for chunkID in decodeDelaySec[uID]:
					chunkIdAll[tileResKey][chunkLength].append(chunkID+1)
					...
					decodeDelayAll[tileResKey][chunkLength].append(normalizeFramesToDecode)
			print len(decodeDelayAll[tileResKey][chunkLength])
    ...
    for chunkLength in lengths:
	    ...
	    pos = l
	    xticks.append(l+lw)
	    for tileRes in sorted(decodeDelayAll,reverse=True):
			boxprops = dict(linewidth=2, color=colors[c],facecolor='white')
			boxplot=plt.boxplot(decodeDelayAll[tileRes][chunkLength],showfliers=False,positions=[pos],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)
			modifybox(boxplot,decodeDelayAll[tileRes][chunkLength],[10,90],2,colors[c],'k',styles[c])
			pos +=lw
			c-=1
	    l+=1
    ...
    plt.ylabel('Decoded area normalized to frame size',fontweight="bold",size=14)
    ...
    plt.ylim(0,2.7)
    #plt.show()
    plt.savefig("graphs/vID "+str(video)+"_decode_delay_chunkLength_boxplot.png",dpi=300)
	#plt.close()
