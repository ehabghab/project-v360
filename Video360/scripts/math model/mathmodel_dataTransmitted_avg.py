		...
		for uID in dis:
			for chunkId in dis[uID]:
				...
				for displacement in displacements:
					if disAvg.get(displacement) is None:
						disAvg[displacement] = 0
					disAvg[displacement] += dis[uID][chunkId][displacement]

		for displacement in disAvg:
			disAvg[displacement] = disAvg[displacement] / numOfChunks

		return disAvg

def getNumberOfTilesAVG(dis,disAvg,tileWidth,tileHeight):
	numOfTiles = {}
	for uID in dis:
		numOfTiles[uID] = {}
		for chunkId in dis[uID]:
			...
			centerLeft = yaw - disAvg['left']
			centerRight = yaw + disAvg['right']
			centerUp = pitch + disAvg['up']
			centerDown = pitch - disAvg['down']
...
def main():
    ...
    for tileWidth,tileHeight in zip(Widths,Heights):
		...
		for chunkLength in Lengths:
		    ...
		    avgDis = getAvgDis(dis)
		    numOfTiles = getNumberOfTilesAVG(dis,avgDis,tileWidth,tileHeight)
    ...
    plt.title("vID:"+str(video)+" User ID:"+str(userID)+" (Math model) avg displacement")
    plt.xlim(-1,60)
    plt.savefig("graphs/vID "+str(video)+"_mathmodel_transmittedBytes_avg.png",dpi=300)
    #plt.show()
