...
def scheme_per_chunk(total_bytes):
	...
	best_scheme= {} #key is res, value is the number of times this scheme is better.
	num_of_chunks = 0.
	for chunk_id in total_bytes_median_reordered:
		...
		for res in total_bytes_median_reordered[chunk_id]:
			if val == -1 or val > total_bytes_median_reordered[chunk_id][res]:
				...
				resolution = res
		if best_scheme.get(resolution) is None:
			best_scheme[resolution] = 0
		best_scheme[resolution] += 1.

	return best_scheme,num_of_chunks
...
def main():
	...
	size_threshold = {'6x6':1,'12x12':1.1,'24x18':1.25}
	...
	for video_id in range(1,31):
		...
		best_scheme,num_of_chunks = scheme_per_chunk(total_bytes)
		for scheme in best_scheme:
			best_scheme_median[video_id][scheme] = (best_scheme[scheme] / num_of_chunks)*100
	...
	plt.subplot(111)
	#plt.grid(zorder=0)
	colors = {'6x6':'g','12x12':'r','24x18':'b'}
	hatches = {'6x6':'x','12x12':'//','24x18':''}
	vi_count = 0
	for video_id in best_scheme_median:
		prev = 0
		for tileWidth,tileHeight in zip((widths),(heights)):
			scheme = str(360/tileWidth)+"x"+str(180/tileHeight)
			if best_scheme_median[video_id].get(scheme) is not None:
				if vi_count == 0:
					plt.bar(vi_count,best_scheme_median[video_id][scheme],bottom=prev,color = colors[scheme],hatch=hatches[scheme],width = .5,label=scheme)
					prev += best_scheme_median[video_id][scheme]
				else:
					plt.bar(vi_count,best_scheme_median[video_id][scheme],bottom=prev,color = colors[scheme],hatch=hatches[scheme],width = .5)
					prev += best_scheme_median[video_id][scheme]
	...
	plt.xlabel('Video Id',fontweight="bold",size=10)
	...
	plt.savefig("graphs/better_scheme_bar_per_video.png",dpi=300,bbox_inches='tight',pad_inches=.05)
	plt.close()
