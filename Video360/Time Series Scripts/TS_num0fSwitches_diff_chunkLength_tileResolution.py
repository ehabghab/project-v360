...
def main():
    ...
    for tileWidth,tileHeight in zip(Widths,Heights):
		for chunkLength in Lengths:
			...
			for uID in switchedTiles:
				...
				for chunkID in switchedTiles[uID]:
					if len(switchedTiles[uID][chunkID]) != 0:
						numOfSwitches[uID][chunkID] = len(switchedTiles[uID][chunkID])


			count = 1000.0/chunkLength
			numOfSwitchesSec = {}

			for uID in numOfSwitches:
				numOfSwitchesSec[uID] = {}
				for chunkID in numOfSwitches[uID]:
			...
			if numOfSwitchesAll.get(keyTileRes,None) == None:
				numOfSwitchesAll[keyTileRes] = {}
				chunkIdAll[keyTileRes] = {}

			numOfSwitchesAll[keyTileRes][chunkLength] = list()
			chunkIdAll[keyTileRes][chunkLength] = list()

			for uID in numOfSwitchesSec:
				for chunkID in numOfSwitchesSec[uID]:
					chunkIdAll[keyTileRes][chunkLength].append(chunkID+1)
					numOfSwitchesAll[keyTileRes][chunkLength].append(numOfSwitchesSec[uID][chunkID])
    ...
    markers=['o','x','v']
    c = 2
    for tileRes in sorted(numOfSwitchesAll,reverse=True):
		for chunkLength in numOfSwitchesAll[tileRes]:
			tileWidth = int(tileRes.split('x')[0])
			tileHeight = int(tileRes.split('x')[1])
			plt.scatter(chunkIdAll[tileRes][chunkLength],numOfSwitchesAll[tileRes][chunkLength],linewidth=2,marker=markers[c],color=colors[c],label = str(360/tileWidth)+"x"+str(180/tileHeight))
			c -=1
    xticksL = list()
    ...
    plt.ylabel('No. of switches',fontweight="bold",size=14)
    ...
    #plt.ylim(0,9)
    plt.show()
    #plt.savefig("graphs/vID "+str(video)+" uID "+str(uID)+"_chunk length "+str(chunkLength)+"_ms_switches.png",dpi=300)
    #plt.close()
	'''
    ...
    for tileRes in sorted(numOfSwitchesAll,reverse=True):
		...
		c = 0
		for chunkLength in sorted(numOfSwitchesAll[tileRes]):
			tileWidth = int(tileRes.split('x')[0])
			tileHeight = int(tileRes.split('x')[1])
			print tileRes
			print chunkLength
			print sum(numOfSwitchesAll[tileRes][chunkLength])
			print "========"
			if xPos==1:
				plt.scatter(xPos,sum(numOfSwitchesAll[tileRes][chunkLength]),linewidth=2,marker=markers[c],color=colors[c],label = str(Lengths[c])+"ms")
			else:
				plt.scatter(xPos,sum(numOfSwitchesAll[tileRes][chunkLength]),linewidth=2,marker=markers[c],color=colors[c])
    ...
    xticksL = list()
    for tileRes in sorted(numOfSwitchesAll,reverse=True):
		xticksL.append(tileRes)

    plt.yticks(fontweight="bold",size=14)
    plt.ylabel('No. of switches',fontweight="bold",size=14)
    plt.legend(loc='best',prop={'size': 14,'weight':'bold'})
    plt.xlabel('Scheme',fontweight="bold",size=14)
    plt.xticks(xticksPos,xticksL,fontweight="bold",size=14)

    #plt.title("vID:"+str(video)+", uID:"+str(uID)+", "+str(chunkLength) +"ms, "+str(bandwidth)+"mbps")
    #plt.xlim(-1,61)
    #plt.ylim(0,9)
    #plt.show()
    plt.savefig("graphs/vID"+str(video)+"_uID "+str(uID)+"_ms_switches.png",dpi=300)
    #plt.close()
