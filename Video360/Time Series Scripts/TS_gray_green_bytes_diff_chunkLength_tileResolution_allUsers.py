...
def main():
    ...
    for tileWidth,tileHeight in zip(Widths,Heights):
		for chunkLength in Lengths:
			...
			greenAll[tileResKey][chunkLength] = {}
			grayAll[tileResKey][chunkLength] = {}
			for uID in grayOver1Sec:
				for chunkID in grayOver1Sec[uID]:
					if greenAll[tileResKey][chunkLength].get(chunkID, None) == None:
						greenAll[tileResKey][chunkLength][chunkID] = list()
						grayAll[tileResKey][chunkLength][chunkID] = list()
					greenAll[tileResKey][chunkLength][chunkID].append((8.*greenOver1Sec[uID][chunkID])/1e6)
					grayAll[tileResKey][chunkLength][chunkID].append((8.*grayOver1Sec[uID][chunkID])/1e6)
    ...
    xaxis = {}
    #print greenAll
    for tileResKey in greenAll:
	    greenAllP50[tileResKey] = {}
	    greenAllP90[tileResKey] = {}
	    greenAllStd[tileResKey] = {}
	    grayAllP50[tileResKey] = {}
	    grayAllP90[tileResKey] = {}
	    grayAllStd[tileResKey] = {}
	    xaxis[tileResKey] = {}
	    for chunkLength in greenAll[tileResKey]:
		    ggray = list()
		    greenAllP50[tileResKey][chunkLength] = list()
		    greenAllP90[tileResKey][chunkLength] = list()
		    greenAllStd[tileResKey][chunkLength] = list()
		    grayAllP50[tileResKey][chunkLength] = list()
		    grayAllP90[tileResKey][chunkLength] = list()
		    grayAllStd[tileResKey][chunkLength] = list()
		    xaxis[tileResKey][chunkLength] = list()
		    for chunkID in sorted(greenAll[tileResKey][chunkLength]):
				chunkDataGreen = greenAll[tileResKey][chunkLength][chunkID]
				greenAllP50[tileResKey][chunkLength].append(np.percentile(chunkDataGreen,50))
				greenAllP90[tileResKey][chunkLength].append(np.percentile(chunkDataGreen,90))
				greenAllStd[tileResKey][chunkLength].append(np.std(chunkDataGreen))
				chunkDataGray = grayAll[tileResKey][chunkLength][chunkID]
				grayAllP50[tileResKey][chunkLength].append(np.percentile(chunkDataGray,50))
				grayAllP90[tileResKey][chunkLength].append(np.percentile(chunkDataGray,90))
				grayAllStd[tileResKey][chunkLength].append(np.std(chunkDataGray))
				xaxis[tileResKey][chunkLength].append(chunkID)
				for val in chunkDataGray:
					ggray.append(val)
		    print np.percentile(ggray,90)
    ...
    for chunkLength in Lengths:
	    ...
	    #plt.grid(axis='y')
	    c = 2
	    for tileRes in sorted(grayAllP50,reverse=True):
			tileWidth = int(tileRes.split('x')[0])
			tileHeight = int(tileRes.split('x')[1])
			label = str(360/tileWidth) +"x"+str(180/tileHeight)
			plt.scatter(xaxis[tileRes][chunkLength][:-1],grayAllP50[tileRes][chunkLength][:-1],linewidth=3,color=colors[c],marker = markers[c],label=label)
			c-=1
	    ...
	    plt.ylim(1.5,5)
	    plt.savefig("graphs/vID "+str(video)+"_Bytes_gray_P50_ts.png",dpi=300)
	    plt.show()
