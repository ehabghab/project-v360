...
def main():
	...
	for video_id in range(1,31):
		...
		displacement_better_scheme = {} #key is res, value is the displacmenet when res is better.
		#chunk_length, user_id, chunk_id, direction --> value.
			...
			for chunk_id in total_bytes_reordered[user_id]:
				...
				for res in total_bytes_reordered[user_id][chunk_id]:
					if val == -1 or val > total_bytes_reordered[user_id][chunk_id][res]:
						...
						resolution = res 
				if displacement_better_scheme.get(resolution) is None:
					displacement_better_scheme[resolution] = []
				if all_.get(resolution) is None:
					all_[resolution] = []
				disp = displacement_1sec[Lengths[0]][user_id][chunk_id]['left'] + displacement_1sec[Lengths[0]][user_id][chunk_id]['right']
				displacement_better_scheme[resolution].append(disp)
				all_[resolution].append(disp)
				if resolution == "6x6" and disp <= 5:
					print(str(video_id)+":"+str(user_id)+":"+str(chunk_id))
		...
		total = 0
		for scheme in displacement_better_scheme:
			total += len(displacement_better_scheme[scheme])


		c = 0
		schemes = ['6x6','12x12','24x18']
		for scheme in schemes:
			frac = "%.2f"%(len(displacement_better_scheme[scheme]) * 1. / total)
			sorted_data = np.sort(displacement_better_scheme[scheme])
			yvals=np.arange(len(sorted_data))/float(len(sorted_data)-1)
			plt.plot(sorted_data,yvals,linewidth=2,color=colors[c],linestyle=line_style[c],label=scheme+"("+frac+")")
			c += 1
		...
		plt.savefig("graphs/_vID_"+str(video_id)+",CL_"+str(Lengths[0])+"ms_displacement_of_better_scheme.png",dpi=300,bbox_inches='tight',pad_inches=.05)
		plt.close()'''
	plt.figure(figsize=(6,4))
	plt.subplot(111)
	plt.grid(zorder=0)
	...
	total = 0
	for scheme in all_:
		total += len(all_[scheme])

	print (total)
	c = 0
	schemes = ['6x6','12x12','24x18']
	for scheme in schemes:
		frac = "%.2f"%(len(all_[scheme]) * 1. / total)
		sorted_data = np.sort(all_[scheme])
		yvals=np.arange(len(sorted_data))/float(len(sorted_data)-1)
		plt.plot(sorted_data,yvals,linewidth=2,color=colors[c],linestyle=line_style[c],label=scheme+"("+frac+")")
		c += 1
	...
	plt.ylabel('Fraction of chunks',fontweight="bold",size=10)
	plt.xlabel('Displacement over yaw',fontweight="bold",size=10)
	plt.xticks([10,20,30,40,50,60,70,80,90],fontweight="bold",size=10)
	plt.legend(loc='best',prop={'size': 10,'weight':'bold'})
	plt.ylim(0,1)
	plt.xlim(0,100)

	#plt.title("Video id:"+str(video_id),size=12,fontweight="bold")
	plt.savefig("graphs/_all_videos_displacement_of_better_scheme.png",dpi=300,bbox_inches='tight',pad_inches=.05)
	plt.close()
