...
def main():
	...
	for uID in framesW:
		for sec in framesW[uID]:
			for tile in framesW[uID][sec]:
				...
				tTimeL[tile][sec].append(framesW[uID][sec])
	...
	maxx = -1
	for tile in tnumU:
		for sec in tnumU[tile]:
			data[tile][sec] = tnumU[tile][sec]/30.0
    ...
    plt.savefig('heatmap_fractionOfUsers_tile_sec_VID'+str(video)+'.png', dpi=300, format='png', bbox_inches='tight') # use format='svg' or 'pdf' for vectorial pictures
    plt.show()
