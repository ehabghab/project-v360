...
def plotMap(tileMap, cx, cy, tileWidth, tileHeight, fname):
    ...
    plt.scatter(cx, cy, linewidth='2', marker='x', color='r', zorder=3)
    FOV = 120
    colorV = 'b'
    for test in range(0, 2):
        ...
        plt.scatter(x6,
                    ...
                    color=colorV,
                    linewidth='2',
                    marker='.' if x6 == x2 else "^",
                    zorder=3)
        FOV = 100
        colorV = 'r'
    plt.savefig("graphs/frame_" + str(fname) + ".png",
                dpi=300,
                bbox_inches='tight',
                pad_inches=.05)
    plt.close()
...
def getTiles(map, cx, cy, tileWidth, tileHeight):
    ...
    if not upflipped and not downflipped:
        ...
        if xul < xur:  #no overlapping
            for i in range(int(xul + tileWidth), int(xur - tileWidth),
                           tileWidth):
                w = tileWidth
                lu = y2 - (yu - tileHeight)
                ld = (yd + tileHeight) - y1
        ...
        if xdl < xdr:
            for i in range(int(xdl + tileWidth), int(xdr - tileWidth),
                           tileWidth):
                w = tileWidth
                ld = (yd + tileHeight) - y1
                tileAreaUsed[map[i][yd + tileHeight]] = w * ld
...
def get_tile_frame_sizes_threshold(tile_width, tile_height, chunk_length,
                                   size_threshold):
    if chunk_length == 200:
    ...
    for i in range(0, 6):  #row
        for j in range(0, 6):
            ...
            dirTile = dir + "/encoded_payloadExtract/" + str(
                i + 1) + "_c_" + str(j + 1)
    ...
    for tile in tile_frame_sizes:
        for frame_num in tile_frame_sizes[tile]:
            ...
            frame_sizes[
                frame_num] = 41070  # avg 6x6 frame size.     #tile_frame_sizes[tile][frame_num] * 1.
    ...
    for row in rows:
        ...
        if (timestampCal - prevFrameTime) >= 40 or prevFrameTime == 0:
            ...
            if chunkId not in l:
                ...
                left, right, flipHorizantal = yawMovement(
                    yaw, prevYaw, startYaw, left, right, flipHorizantal)
                down, up, flipVertical = pitchMovement(pitch, prevPitch,
                                                       startPitch, up, down,
                                                       flipVertical)
                prevYaw = yaw
                prevPitch = pitch
...
def plot_map(data, video, chunkLength, tileWidth, tileHeight, tileMap, targ):
        ...
        for uID in data[vID]:
            ...
            for row in data[vID][uID]:
                ...
                if (timestampCal - prevFrameTime) >= 40 or prevFrameTime == 0:
                    ...
                    if frameNumT == targ:
                        cx = yaw  #.append(yaw)
                        cy = pitch  #append(pitch)
                        break
    plotMap(tileMap, cx, cy, tileWidth, tileHeight, targ)
...
def main():
    ...
    for video_id in range(1, 2):
        ...
        data = get_raw_data(video_id, 3)

        chunkLength = 1000  #chunk length in millisecond
        widths = [30]
        heights = [15]
        area_waste = {
        }  #K1: tile resolution, K2:user id, K3:chunk id --> V: area wasted among watched frames within tiles.
        drop_frame_waste = {}
        waste = {}
        total_bytes = {}
        for tileWidth, tileHeight in zip((widths), (heights)):
            ...
            tileFrameSizes = get_tile_frame_sizes_threshold(
                (360 / tileWidth), (180 / tileHeight), chunkLength,
                size_threshold)
            tileMap = generateMAP(tileWidth, tileHeight)

            for i in range(1370, 1385):
                plot_map(data,video_id,chunkLength,\
                 tileWidth,tileHeight,tileMap,i)
