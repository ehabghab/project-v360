...
def getNumberOfTiles(dis,tileWidth,tileHeight):
		...
		for uID in dis:
			...
			for chunkId in dis[uID]:
				...
				centerLeft = yaw - dis[uID][chunkId]['left']
				centerRight = yaw + dis[uID][chunkId]['right']
				centerUp = pitch + dis[uID][chunkId]['up']
				centerDown = pitch - dis[uID][chunkId]['down']
...
'''def getNumberOfTiles(dis,tileWidth,tileHeight):
	...
	#plotMap(tileMap,330,105,tileWidth,tileHeight)
	numOfTiles = {}
	for uID in dis:
		numOfTiles[uID] = {}
		for chunkId in dis[uID]:
			numOfTiles[uID][chunkId] = 0
			tileSet = set()
			yaw = dis[uID][chunkId]['yaw']
			pitch = dis[uID][chunkId]['pitch']
			...
			#print(str(y1)+"-->"+str(y2))
			if x1 > x2:
				for i in range(x1, 360, 1):
					if y1 > y2:
						for j in range(y1, 180, 1):
							tiles, wastes = getTiles(tileMap,i,j,tileWidth,tileHeight)
							for tile in tiles:
								tileSet.add(tile)
						for j in range(tileHeight/2, y2+1, 1):
							tiles, wastes = getTiles(tileMap,i,j,tileWidth,tileHeight)
							for tile in tiles:
								tileSet.add(tile)
				...
				for i in range(tileWidth/2, x2+1, 1):
					if y1 > y2:
						for j in range(y1, 180, 1):
							tiles, wastes = getTiles(tileMap,i,j,tileWidth,tileHeight)
							for tile in tiles:
			...
			numOfTiles[uID][chunkId] = len(tileSet)
	return numOfTiles'''
...
def main():
    ...
    for tileWidth,tileHeight in zip(Widths,Heights):
		...
		for chunkLength in Lengths:
		    ...
		    numOfTiles = getNumberOfTiles(dis,tileWidth,tileHeight)
		    for uID in numOfTiles:
				normalizedArea[scheme][chunkLength][uID] = list()
				xAxis[scheme][chunkLength][uID] = list()
				for chunkId in numOfTiles[uID]:
					if chunkSizes.get(chunkId) is None:
						continue
					normalizedArea[scheme][chunkLength][uID].append((numOfTiles[uID][chunkId] * tileWidth * tileHeight) / (360.*180.))
    ...
    for chunkLength in Lengths:
	    for tileWidth,tileHeight in zip(Widths,Heights):
			scheme = str(360/tileWidth)+"x"+str(180/tileHeight)
			for uID in normalizedArea[scheme][chunkLength]:
				plt.scatter(xAxis[scheme][chunkLength][uID],normalizedArea[scheme][chunkLength][uID],linewidth=2,marker=styles[c],color=colors[c],label = scheme)
			c -= 1
    ...
    plt.title("vID:"+str(video)+" User ID:"+str(userID)+" (Math model)")
    plt.xlim(-1,60)
    plt.savefig("graphs/vID "+str(video)+"_userID "+str(userID)+"_mathmodel_normalizedArea.png",dpi=300)
    #plt.show()
