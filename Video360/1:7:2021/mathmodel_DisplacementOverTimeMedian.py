...
def main():
	...
	for video_id in range(1,31):
		...
		plt.ylabel('Displacement over yaw (all users) ',fontweight="bold",size=10)
		plt.xlabel('Chunk id',fontweight="bold",size=10)
		plt.xticks(xlabel,xlabel,fontweight="bold",size=10)
		plt.legend(loc='best',prop={'size': 10,'weight':'bold'})
		plt.ylim(-2,110)

		plt.title("Video id:"+str(video_id)+", Chunk length:"+str(Lengths[0])+"ms",size=12,fontweight="bold")
		plt.xlim(-0.5,59.5)
		plt.savefig("graphs/vID_"+str(video_id)+",CL_"+str(Lengths[0])+"ms.png",dpi=300,bbox_inches='tight',pad_inches=.05)
		#plt.show()
		plt.close()
