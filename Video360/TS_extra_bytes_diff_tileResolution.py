...
def main():
    ...
    #tileSize = generateTileSizes(frameSizes,frameTypes,numberOfTiles,chunkLength)
	...
	for uID in framesWatchedInTile:
		...
		for chunkID in framesWatchedInTile[uID]:
			...
			# bytes in wasted area within tile frames (watched frames).
			'''for fovNum in tilesInFOV[uID][chunkID]:
				if tileSize.get(baseFrameNum+fovNum,None) != None:
					s = tileSize[baseFrameNum+fovNum]
					ideal[uID][chunkID] += (s)*((10000.)/(tileWidth*tileHeight))
					extraArea = (tilesInFOV[uID][chunkID][fovNum]*tileWidth*tileHeight) - 10000.0
					wastedAreInTileFrames += (s)*(extraArea/(tileWidth*tileHeight))'''
	...
	for uID in wasteOver1Sec:
		...
		for chunkID in wasteOver1Sec[uID]:
			xaxis.append(chunkID)
			waste = float(wasteOver1Sec[uID][chunkID])
			ywastage.append(waste/1e6)

		xticks = list()
		for i in range (0,chunkID + 4,5):
			xticks.append(i)

		#plt.scatter(xaxis,ywastage,linewidth=2,marker='^',color='magenta',label = str(360/tileWidth)+"X"+str(180/tileHeight))
	...
	for uID in wasteOver1Sec:
		...
		#plt.scatter(xaxis,ywastage,linewidth=2,marker='v',color='b',label = str(360/tileWidth)+"X"+str(180/tileHeight))
		...
		for chunkID in framesWatchedInTile[uID]:
			...
			for tile in framesWatchedInTile[uID][chunkID]:
				for fId in range(1,framesInTile[uID][chunkID]+1):
					if fId not in framesWatchedInTile[uID][chunkID][tile]:
						if tileFrameSizes.get(baseFrameNum+fId,None) != None:
							wastedTileFrames += tileFrameSizes[tile][baseFrameNum+fId] #tileSize[baseFrameNum+fId]
	...
	for uID in wasteOver1Sec:
		...
		plt.scatter(xaxis,ywastage,linewidth=2,marker='x',color='red',label = str(360/tileWidth)+"X"+str(180/tileHeight))
    ...
    for uID in wasteOver1Sec:
		...
		#plt.scatter(xaxis,ywastage,linewidth=2,marker='o',color='g',label = str(360/tileWidth)+"X"+str(180/tileHeight))


		plt.title("vID:"+str(video)+" uID:"+str(uID))#+" [width "+str(tileWidth)+" X height"+str(tileHeight)+"], chunk length "+str(chunkLength) +"ms")
		plt.xticks(xticks,fontweight="bold",size=14)
		...
		plt.ylabel('Extra data transmitted (MB)',fontweight="bold",size=14)
		plt.xlim(-1,chunkID + 1)
		plt.ylim(-.0125,.15+0.0125)
		#plt.show()
		plt.savefig("graphs/vID "+str(video)+" uID "+str(uID)+"_chunk length "+str(chunkLength)+"_ms_ts.png",dpi=300,bbox_inches='tight', pad_inches=0)
		#plt.close()
