...
def main():
    ...
    colors2 = {'6x6':'deepskyblue','12x12':'lightsalmon','24x18':'lightgreen'}
    ...
    for video in sum_median_bytes:
		    ...
		    if video % 2 ==0:
			    if video == startV:
				    for key in res:
					    	plt.scatter(x[key],res[key],marker = markers[key],color=colors[key],s=10,label = key)
			    else:
				    for key in res:
					    	plt.scatter(x[key],res[key],marker = markers[key],color=colors[key],s=10)

		    else:
			    if video == startV:
				    for key in res:
					    	plt.scatter(x[key],res[key],marker = markers[key],color=colors2[key],s=10,label = key)
			    else:
				    for key in res:
					    	plt.scatter(x[key],res[key],marker = markers[key],color=colors2[key],s=10)

		    #else:
			#    for key in res:
			#    	plt.scatter(x[key],res[key],marker = markers[key],color=colors2[key],s=10)
		    plt.axvline(x=(video - startV) * 60-.5,linestyle='--',linewidth = 0.3)
		    ...
		    plt.xlim(-1,((endV-startV)+1)*60)
		    #plt.grid(True)


		   # plt.ylim()
		    #plt.show()
		    plt.savefig("graphs/2data_transmitted_all_user_all_videos_"+str(startV)+".png",dpi=300,bbox_inches='tight', pad_inches=0)
			#plt.close()
