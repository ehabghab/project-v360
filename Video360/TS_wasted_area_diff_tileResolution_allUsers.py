...
def main():
	...
	for j in range(1,15):
		...
		for i in range(startV,endV+1):
			...
			sum_bytes[video],sum_median_bytes[video] = sum_over_users(total_bytes)
		...
		plt.subplot(111)
		colors = {'6x6':'b','12x12':'r','24x18':'g'}
		markers = {'6x6':'^','12x12':'v','24x18':'o'}

		colors2 = {'6x6':'b','12x12':'r','24x18':'g'}#{'6x6':'deepskyblue','12x12':'lightsalmon','24x18':'lightgreen'}


		keys = ['6x6','12x12','24x18']
		for video in sum_median_bytes:
				res = {}
				x = {}
				for key in keys:
					res[key] = []
					x[key] = []
					for i in range(0,60):
						res[key].append(sum_median_bytes[video][key][i])
						x[key].append(i + (video - startV) * 60)

				if video % 2 ==0:
					if video == startV:
						for key in res:
								plt.scatter(x[key],res[key],marker = markers[key],color=colors[key],s=10,label = key,zorder=3)
					else:
						for key in res:
								plt.scatter(x[key],res[key],marker = markers[key],color=colors[key],s=10,zorder=3)

				else:
					if video == startV:
						for key in res:
								plt.scatter(x[key],res[key],marker = markers[key],color=colors2[key],s=10,label = key,zorder=3)
					else:
						for key in res:
								plt.scatter(x[key],res[key],marker = markers[key],color=colors2[key],s=10,zorder=3)

				plt.yticks(fontweight="bold",size=10)
				plt.ylabel('Data transmitted on median (MB)',fontweight="bold",size=10)
				plt.legend(loc='best',prop={'size': 8,'weight':'bold'},ncol =3)
				...
				plt.title("Video id:"+str(video),size=12,fontweight="bold")

			   # plt.ylim()
				#plt.show()
				plt.savefig("graphs/data_transmitted_all_user_all_videos_"+str(startV)+".png",dpi=300,bbox_inches='tight', pad_inches=0)
				#plt.close()
