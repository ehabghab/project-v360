...
def plotMap(tileMap,cx,cy,tileWidth,tileHeight,fname):
	...
	plt.savefig("graphs/"+fname+".png",dpi=300,bbox_inches='tight',pad_inches=.05)
	plt.close()
...
def plot_map(data,video,chunkLength,tileWidth,tileHeight,tileMap,user_id,chunk_id):
	...
	for vID in data:
		...
		for uID in data[vID]:
			...
			for row in data[vID][uID]:
				...
				if (timestampCal - prevFrameTime) >=40 or prevFrameTime == 0:
					prevFrameTime += 40
					if uID == user_id and chunk_id == chunkID:
						fname= str(vID)+"_"+str(uID)+"_["+str(chunkID)+"]-["+str(tileHeight)+"] "+str(FOVNum)
						plotMap(tileMap,yaw,pitch,tileWidth,tileHeight,fname)
					FOVNum +=1
	...
	for video_id in range(3,4):
		...
		for tileWidth,tileHeight in zip((widths),(heights)):
			...
			plot_map(data,video_id,chunkLength,\
				tileWidth,tileHeight,tileMap,52,40)
