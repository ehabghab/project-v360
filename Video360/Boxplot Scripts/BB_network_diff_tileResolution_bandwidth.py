...
def main():
    ...
    for tileWidth,tileHeight in zip((widths),(heights)):
		for bandwidth in bandwidths:
			...
			networkDelayForSwitchedTiles = {} #key is chunkID, value is transmission delay caused by switch.
			...
			if networkDelayAll.get(tileResKey,None) == None:
				...
				tileResKeys.append(tileResKey)

			networkDelayAll[tileResKey][bandwidth] = list()
			chunkIdAll[tileResKey][bandwidth] = list()
			for uID in networkDelaySec:
				for chunkID in networkDelaySec[uID]:
					chunkIdAll[tileResKey][bandwidth].append(chunkID+1)
					networkDelayAll[tileResKey][bandwidth].append(networkDelaySec[uID][chunkID]*1000.)#ms
    ...
    for bandwidth in bandwidths:
	    ...
	    pos = l
	    xticks.append(l+lw)
	    for tileRes in sorted(networkDelayAll,reverse=True):
			boxprops = dict(linewidth=2, color=colors[c],facecolor='white')
			boxplot=plt.boxplot(networkDelayAll[tileRes][bandwidth],showfliers=False,positions=[pos],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)
			modifybox(boxplot,networkDelayAll[tileRes][bandwidth],[10,90],2,colors[c],'k',styles[c])
			pos +=lw
			#plt.scatter(chunkIdAll[tileRes][9],networkDelayAll[tileRes][9],linewidth=2,marker=markers[c],color=colors[c],label = str(360/tileWidth)+"x"+str(180/tileHeight))
			c-=1
	    l+=1
    ...
    plt.ylabel('Netowrk Latency (ms)',fontweight="bold",size=14)
    plt.legend(handles= matches, loc='best',prop={'size': 14,'weight':'bold'})
    plt.xlabel('Bandwith',fontweight="bold",size=14)
    plt.xticks(xticks,xlabels,fontweight="bold",size=14)

    plt.title("vID:"+str(video)+", uID:"+str(uID)+", "+str(chunkLength) +"ms")
    plt.xlim(0)
    plt.ylim(0,150)
    #plt.show()
    plt.savefig("graphs/vID "+str(video)+" uID "+str(uID)+"_chunk length "+str(chunkLength)+"_ms_network_delay_bandwidth_boxplot.png",dpi=300)
	#plt.close()
