...
def main():
    ...
    for chunkLength in Lengths:
	    ...
	    for tileRes in sorted(unBytesAll,reverse=True):
			print len(unBytesAll[tileRes][chunkLength])
			boxprops = dict(linewidth=2, color=colors[c],facecolor='white')
			boxplot=plt.boxplot(unBytesAll[tileRes][chunkLength],showfliers=False,positions=[pos],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)
			modifybox(boxplot,unBytesAll[tileRes][chunkLength],[10,90],2,colors[c],'k',styles[c])
			pos +=lw
			c-=1
	    l+=1
    ...
    plt.ylabel('Extra bytes (MB)',fontweight="bold",size=14)
    ...
    plt.ylim(0,1)
    plt.savefig("graphs/vID "+str(video)+"_extra_bytes_boxplot.png",dpi=300)
    #plt.show()
