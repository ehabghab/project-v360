...
def main():
    ...
    for tileWidth,tileHeight in zip(Widths,Heights):
		for chunkLength in Lengths:
		    ...
		    framesInTile = {} # number of frames in chunk, key is (user id --> chunkNum), value number of frames within tile.
		                ...
		                if (timestampCal - prevFrameTime) >=40 or prevFrameTime == 0:
		                    ...
		                    for tile in tiles:
		                        if framesWatchedInTile[uID][chunkID].get(tile,None) == None:
		                            framesWatchedInTile[uID][chunkID][tile] = 0
		                        framesWatchedInTile[uID][chunkID][tile] = framesWatchedInTile[uID][chunkID][tile] + 1
			...
			for uID in framesWatchedInTile:
				...
				for chunkID in framesWatchedInTile[uID]:
					...
					#number of dropped frames in a tile (unwatched frames)
					for tile in framesWatchedInTile[uID][chunkID]:
						if (framesWatchedInTile[uID][chunkID][tile] < framesInTile[uID][chunkID]):
							wastedTileFrames += framesInTile[uID][chunkID] - framesWatchedInTile[uID][chunkID][tile]

					# wasted area within tile frames (watched frames).
					for fovNum in tilesInFOV[uID][chunkID]:
						wastedAreInTileFrames += (tilesInFOV[uID][chunkID][fovNum]*tileWidth*tileHeight) - 10000.0
					waste[uID][chunkID] = int( (wastedTileFrames * tileHeight * tileWidth) + wastedAreInTileFrames)
			...
			for uID in waste:
				...
				for chunkID in waste[uID]:
					...
					if wasteOver1Sec[uID].get(chunkSumID,None) == None:
						...
						noFrameOver1Sec[uID][chunkSumID] = 0
					wasteOver1Sec[uID][chunkSumID] += waste[uID][chunkID]
					noFrameOver1Sec[uID][chunkSumID] += framesInTile[uID][chunkID]




			keyTileRes = str(tileWidth)+"x"+str(tileHeight)
			if wastagePixelsAll.get(keyTileRes,None) == None:
				wastagePixelsAll[keyTileRes] = {}
				chunkIdAll[keyTileRes] = {}

			wastagePixelsAll[keyTileRes][chunkLength] = list()
			chunkIdAll[keyTileRes][chunkLength] = list()


			for uID in wasteOver1Sec:
				for chunkID in wasteOver1Sec[uID]:
					chunkIdAll[keyTileRes][chunkLength].append(chunkID)
					waste = float(wasteOver1Sec[uID][chunkID])/(noFrameOver1Sec[uID][chunkID]*10000.)
					wastagePixelsAll[keyTileRes][chunkLength].append(waste*100)
    ...
    for chunkLength in Lengths:
	    ...
	    pos = l
	    xticks.append(l+lw)
	    for tileRes in sorted(wastagePixelsAll,reverse=True):
			boxprops = dict(linewidth=2, color=colors[c],facecolor='white')
			boxplot=plt.boxplot(wastagePixelsAll[tileRes][chunkLength],showfliers=False,positions=[pos],widths=[.1],whis=[10,90], boxprops=boxprops,patch_artist=True)
			modifybox(boxplot,wastagePixelsAll[tileRes][chunkLength],[10,90],2,colors[c],'k',styles[c])
			pos +=lw
			c-=1
	    l+=1
    ...
    plt.ylabel('% of unnecessary pixels',fontweight="bold",size=14)
    ...
    plt.title("vID:"+str(video)+", uID:"+str(uID))
    #plt.ylim(0,150)
    plt.savefig("graphs/vID "+str(video)+" uID "+str(uID)+"_Per_Pixels_boxplot.png",dpi=300)
