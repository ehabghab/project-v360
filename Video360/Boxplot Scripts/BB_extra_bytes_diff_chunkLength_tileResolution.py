...
def main():
		...
		for chunkLength in Lengths:
					...
					for tile in framesWatchedInTile[uID][chunkID]: #each tile in chunk duration
						for fId in range(1,framesInTile[uID][chunkID]+1): #for each frame
							...
							if fId in framesWatchedInTile[uID][chunkID][tile]:
								if tileFrameSizes.get(baseFrameNum+fId,None) != None:
			...
			for uID in grayOver1Sec:
				for chunkID in grayOver1Sec[uID]:
					...
					greenAll[tileResKey][chunkLength].append(greenOver1Sec[uID][chunkID])
					grayAll[tileResKey][chunkLength].append(grayOver1Sec[uID][chunkID])
    ...
    for chunkLength in Lengths:
	    ...
	    for tileRes in sorted(grayAll,reverse=True):
			print len(grayAll[tileRes][chunkLength])
    ...
    plt.xlim(0)
    plt.title("vID:"+str(video)+", uID:"+str(userID))
    #plt.ylim(0,150)
    plt.savefig("graphs/vID "+str(video)+" uID "+str(uID)+"_Bytes_gray_green_boxplot.png",dpi=300)
    #plt.show()
