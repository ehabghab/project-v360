...
def main():
    ...
    for dir in dirs:
        ...
        for i in range(0,heightTiles): #row
            for j in range(0,widthTiles):
                ...
                dirTile = dir+"/EXP2/encoded_payloadExtract/"+str(i+1)+"_c_"+str(j+1)
    ...
    for dir in sorted(frameSizes):
        ...
        for frameId in frameSizes[dir]:
            sum += frameSizes[dir][frameId] * 1.
            leng += 1 * 1.
        avg = sum/leng
        print(str(dir)+","+str(leng)+":"+"%.2f"%(avg/1000.)+" KB")
    ...
    plt.title("ChunkLength 1000ms",fontweight="bold",size=16)
    plt.grid(which='both')
    plt.xlim(0,75)
    #plt.ylim(.8,1.01)#,1)
    #plt.show()
